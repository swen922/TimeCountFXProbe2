...
public class ThreadUpdate extends Task<Boolean> {
    ...
    protected Boolean call() {
        ...
        if (serializeWrapper != null) {
            ...
            if ((serializeWrapper.getUpdateType() != null) && (serializeWrapper.getObject() != null)) {
                ...
                AllData.updateAllStatus("ThreadUpdate - Отмена отправки обновления на сервер из-за ошибки в объекте SerializeWrapper. Отсутствуют необходимые объекты в нем. \n" +
                        "SerializeWrapper = " + serializeWrapper);
                AllData.logger.error(AllData.status);
            }
        }
        else {
